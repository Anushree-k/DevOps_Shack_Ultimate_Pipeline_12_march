...
./install_docker.sh
```
By default, after docker is installed only admin has privilege to execute docker commands.
so for all users to execute and run docker - run following command.
```bash
